...
Sprite.prototype.raakIk = function raakIk(sprite){
  let raakt = this.collidesWith(sprite);
  //Move away from object so detection occurs only once
  /*if (raakt){
    this.xv = -this.xv;
    this.yv = -this.yv;
    this.neemStappen(10);
...
Sprite.prototype.raakIkRand = function raakIkRand(){
  if (this.x <= 0 || this.y <= 0 || this.y >=  360 - this.h || this.x >= 480 - this.w){
    return true;
  }else{
    return false;
  }
...
Sprite.prototype.keerOmAanRand = function keerOmAanRand(){
  if (this.raakIkRand()){
    if (this.x <= 0){
      this.x = 1;
      this.xv = -this.xv;
